...
def line(x1, y1, x2, y2):
    ...
    Использует алгоритм Берзенхэма. """
    ...
    # являются в определенном смысле соседними, что данная функция не умеет хорошо
    # обрабатывать, поэтому возвращаем заранее заготовленный список:
    if (x1 == x2 and y1 == y2 + 1) or (y1 == y2 and x1 == x2 + 1):
        return [(x1, y1), (x2, y2)]

    isSteep = abs(y2 - y1) > abs(x2 - x1)
    if isSteep:
        # Этот алгоритм умеет работать только с некрутыми прямыми, так что делаем
        # уклон не таким крутым, а потом возвращаем обратно.
        x1, y1 = y1, x1  # Меняем местами x1 и y1
        x2, y2 = y2, x2  # Меняем местами x2 и y2

    isReversed = x1 > x2  # True, если прямая идет справа налево
    if isReversed:  # Получаем точки прямой идущей справа налево
        ...
        if y1 < y2:
            ...
            ydirection = -1
        # Вычисляем для каждого х в этой прямой:
        ...
        if y1 < y2:
            ...
            ydirection = -1
        # Вычисляем у для каждого х в этой прямой:
        for currentx in range(x1, x2 + 1):
            if isSteep:
                ...
                points.append((currentx, currenty))
            extray -= deltay
            if extray < 0:  # Меняем у, только если extray < 0
                currenty += ydirection
                extray += deltax
    return points
...
def rotatePoint(x, y, z, ax, ay, az):
    ...
    Возвращение на углы ax, ay, az (в радианах) относительно начала координат 0,
    0, 0.
...
def adjustPoint(point):
    ...
    оторбражения на экране. Для этого масштабируем данную двумерную точку для
    SCALEX и SCALEY, а затем сдвигаем ее на TRANSLATEX и TRANSLATEY."""
    return (int(point[X] * SCALEX + TRANSLATEX),
            int(point[Y] * SCALEY + TRANSLATEY))


"""В CUBE_CORNERS хранятся координаты XYZ углов куба.
Индексы всех углов CUBE_CORNERS отмечены на следующей схеме:
    0---1
   /|  /|
  2---3 |
  | 4-| 5
...
CUBE_CORNERS = [[-1, -1, -1], # Точка 0
                ...
                [-1, 1, 1], # Точка 6
                [1, 1, 1]] # Точка 8
# В rotatedCorners хранятся координаты XYZ из CUBE_CORNERS после вращения на rx,
# ry, rz:
...
try:
    while True: # Основной цикл программы
        ...
        for i in range(len(CUBE_CORNERS)):
            ...
            y = CUBE_CORNERS[i][Y]
            z = CUBE_CORNERS[i][Z]
            rotatedCorners[i] = rotatePoint(x, y, z, xRotation, yRotation,
                                            zRotation)

            # Получаем точки линий куба:
            cubePoints = []
            for fromCornerIndex, toCornerIndex in ((0, 1), (1, 3), (3, 2),
            (2, 0), (0, 4), (1, 5), (2, 6), (3, 7), (4, 5), (5, 7),
            (7, 6), (6, 4)):
    ...
    print('RotatingCube, ESHA. shashkow@gmail.com')
